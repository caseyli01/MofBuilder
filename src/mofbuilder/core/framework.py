...
class Framework:
    ...
    def _read_net(self):
        ...
        assert_msg_critical(self.frame_net.cif_file is not None,
    ...
    def _read_linker(self):
        ...
        if self.frame_linker.linker_topic > 2:
            ...
            self.linker_length = np.linalg.norm(self.linker_outer_X_data[0,5:8].astype(float) -
                                                self.linker_outer_X_data[1,5:8].astype(float))
        else:
            self.linker_length = np.linalg.norm(self.linker_center_X_data[0,5:8].astype(float) -
    ...
    def _read_termination(self):
        ...
        if not (Path(self.termination_filename).is_file()):
            ...
            if self._debug:
                self.ostream.print_info(f"Termination file {self.termination_filename} is not a valid file path. Searching in termination database.")
                self.ostream.flush()
            ...
            selected_termination_pdb_filename = fetch_pdbfile(terminations_database_path, keywords,
                                                               nokeywords,self.ostream)[0]
            assert_msg_critical(selected_termination_pdb_filename is not None,
    ...
    def read_framework(self):
        ...
        if self._debug:
            ...
            self.ostream.print_info(f"  Net: {self.mof_family}, spacegroup: {self.spacegroup}, cell: {self.cell_info}")
            self.ostream.print_info(f"  Node: {self.frame_nodes.filename} with metal type {self.node_metal_type}")
            self.ostream.print_info(f"  Linker: {self.frame_linker.linker_topic}")
            if self.termination:
    ...
    def optimize_framework(self):
        ...
        self.ostream.print_info("Start to optimize the node rotations and cell parameters") 
        self.ostream.flush()
