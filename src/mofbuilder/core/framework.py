...
class Framework:
    ...
    def __init__(self, comm=None, ostream=None, mof_family=None):
        ...
        self.frame_terminations = FrameTermination(comm=self.comm, ostream=self.ostream)
        self.frame_net = FrameNet(comm=self.comm, ostream=self.ostream)
        self.mof_top_library = MofTopLibrary(comm=self.comm, ostream=self.ostream)
    ...
    def _read_net(self):
        ...
        self.frame_net.cif_file = self.mof_top_library.fetch(mof_family=self.mof_family)
        assert_msg_critical(self.frame_net.cif_file is not None, "Template cif file is not set in mof_top_library.")
    ...
    def _read_linker(self):
        ...
        if self.frame_linker.linker_connectivity> 2:
            ...
            linker_com = np.mean(self.frame_linker.linker_outer_X_data[:, 5:8].astype(float), axis=0)
            self.linker_outer_data = np.hstack((self.frame_linker.linker_outer_data[:, 0:5], self.frame_linker.linker_outer_data[:, 5:8].astype(float)-linker_com, self.frame_linker.linker_outer_data[:, 8:]))
            self.linker_outer_X_data = np.hstack((self.frame_linker.linker_outer_X_data[:, 0:5], self.frame_linker.linker_outer_X_data[:, 5:8].astype(float)-linker_com, self.frame_linker.linker_outer_X_data[:, 8:]))
            self.linker_frag_length = np.linalg.norm(self.linker_outer_X_data[0, 5:8].astype(float)-self.linker_outer_X_data[1, 5:8].astype(float))
        else:
            self.linker_frag_length = np.linalg.norm(self.linker_center_X_data[0, 5:8].astype(float)-self.linker_center_X_data[1, 5:8].astype(float))

    def _read_node(self):
        assert_msg_critical(self.node_connectivity is not None, "node_connectivity is not set")
        assert_msg_critical(self.node_metal_type is not None, "node_metal_type is not set")
        ...
        selected_node_pdb_filename = fetch_pdbfile(nodes_database_path, keywords, nokeywords, self.ostream)[0]
    ...
    def _read_termination(self):
        ...
        if self.termination_filename is None:
            assert_msg_critical(False, "Termination is set to True but termination_filename is None.")
        ...
        if not (Path(self.termination_filename).is_file()):
            ...
            selected_termination_pdb_filename = fetch_pdbfile(terminations_database_path, keywords, nokeywords, self.ostream)[0]
            assert_msg_critical(
            ...
            self.termination_filename = str(Path(terminations_database_path, selected_termination_pdb_filename))
        if self._debug:
            self.ostream.print_info(f"Using termination file: {self.termination_filename}")
            self.ostream.flush()
    ...
    def read_framework(self):
        ...
        if self._debug:
            ...
            self.ostream.print_info(f"Net: {self.mof_family}, spacegroup: {self.net_spacegroup}, cell: {self.net_cell_info}")
            self.ostream.print_info(f"Node: {self.frame_nodes.filename} with metal type {self.node_metal_type}")
            self.ostream.print_info(f"Linker: {self.frame_linker.linker_connectivity}")
            if self.termination:
                self.ostream.print_info(f"Termination: {self.termination_filename}")
            else:
    ...
    def optimize_framework(self):
        ...
        self.ostream.print_info("--------------------------------")
        self.ostream.print_info("Finished optimizing the node rotations and cell parameters")
        self.ostream.print_separator()
    ...
    def exchange_defects(self,graph):
        ...
        if self.exchange_node_pdbfile is not None:
            ...
            self.defectgenerator.exchange_node_data = pdbreader.read_pdb(filepath=self.exchange_node_pdbfile)
        ...
        if self.exchange_linker_pdbfile is not None:
            ...
            pdbreader = PdbReader(comm=self.comm, ostream=self.ostream)
            self.defectgenerator.exchange_linker_data = pdbreader.read_pdb(filepath=self.exchange_linker_pdbfile)
        ...
        if self.exchange_linker_molecule is not None:
            ...
            if fr_ex_linker.linker_connectivity> 2:
                ...
                ex_linker_com = np.mean(fr_ex_linker.linker_outer_X_data[:, 5:8].astype(float), axis=0)
                ex_linker_outer_data = np.hstack((fr_ex_linker.linker_outer_data[:, 0:5], fr_ex_linker.linker_outer_data[:, 5:8].astype(float)-ex_linker_com, fr_ex_linker.linker_outer_data[:, 8:]))
                ex_linker_outer_X_data = np.hstack((fr_ex_linker.linker_outer_X_data[:, 0:5], fr_ex_linker.linker_outer_X_data[:, 5:8].astype(float)-ex_linker_com, fr_ex_linker.linker_outer_X_data[:, 8:]))
                ex_linker_frag_length = np.linalg.norm(ex_linker_outer_X_data[0, 5:8].astype(float)-ex_linker_outer_X_data[1, 5:8].astype(float))
            else:
                ex_linker_frag_length = np.linalg.norm(ex_linker_center_X_data[0, 5:8].astype(float)-ex_linker_center_X_data[1, 5:8].astype(float))
    ...
    def remove_defects(self,graph):
        ...
        #remove
        rmG = self.defectgenerator.remove_items_or_terminate(self.remove,graph)
        return rmG.copy()
