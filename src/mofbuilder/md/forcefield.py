...
class ForceFieldProcessor:
    ...
    def _reconnect_linker_molecule(self, linker_mol_data_file):
        ...
        atom_coords = []
        with open(linker_mol_data_file, 'r') as f:
            lines = f.readlines()
        ...
        connectivity_matrix = self.reconnect(X_indices_coords, connectivity_matrix)
        connectivity_matrix = self.reconnect(lower_x_indices_coords, connectivity_matrix)
        ff_gen = MMForceFieldGenerator()
        ff_gen.topology_update_flag = True
        ff_gen.connectivity_matrix = connectivity_matrix
        ff_gen.create_topology(molecule, resp=False)
        ff_gen.write_gromacs_files(filename="linker4")
        molecule.write_xyz_file("reconnected_linker.xyz")

    def reconnect(self, X_indices_coords, connectivity_matrix):
        ...
        for i, j in zip(X1_ind_coords, X2_ind_coords):
            ...
            if dist < 4.0: #threshold for x-x bond
                ...
                print(f"X-X distance {dist} is within threshold, bond created between indices {i[0]} and {j[0]}.")
            else:
                continue
        return connectivity_matrix

if __name__ == "__main__":
    processor = ForceFieldProcessor()
    processor._reconnect_linker_molecule('UiO-66_linker.xyz')
    #processor._reconnect_linker_molecule('PCN-222_linker.xyz')
